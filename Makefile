...
coresrc = CPL_module.f90 CPL_methods.f90 commondefs.f90 CPL_io.f90 CPL_write.f90
...
3plibfiles = $(CPL_THIRD_PARTY_LIB)/libjsonfortran.so
...
ifeq ($(3rdpartybuild),true)
		$(LINK) $(LSHAREDLIB) -o $(lib) $(allobjfiles) $(3plibfiles) -L$(CPL_THIRD_PARTY_LIB) -Wl,-rpath $(CPL_THIRD_PARTY_LIB) $(LFLAGS) 
else                                                     
		$(LINK) $(LSHAREDLIB) -o $(lib) $(allobjfiles) $(3plibfiles) -L$(CPL_THIRD_PARTY_LIB) -Wl,-rpath $(CPL_THIRD_PARTY_LIB) $(LFLAGS) 
endif
...
json-fortran: $(CPL_THIRD_PARTY_LIB) $(CPL_THIRD_PARTY_INC)
	...
	# cp -r $(CPL_THIRD_PARTY_INC)/* $(includedir)/
	# cp -r $(CPL_THIRD_PARTY_LIB)/* $(libdir)/
	make
